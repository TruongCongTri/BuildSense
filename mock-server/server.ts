...
const ACTIVE_BUILDINGS: ServerBuilding[] = [
  ...
  },
  {
    ...
    modelUrl:
      "https://services.arcgis.com/HRPe58bUyBqyyiCt/arcgis/rest/services/MKE_Midwest_Express_3D_WSL11/SceneServer",
    bounds: null,
    defaultCamera: null,
  },
];
...
const SENSOR_TYPES: Array<{
  ...
  // Structural displacement and movement
  {
    ...
    locations: ["Foundation", "Main Pillar"],
    zRange: [0.0, 0.3],
  },
  {
    type: "PDS",
    ...
    base: 0.1,
    manufacturer: "Nerve-Sensors",
    locations: ["Floor Base", "Foundation"],
    zRange: [0.0, 0.1],
  },

  // Cracks and structural fatigue
  {
    type: "SmartCrackDetection",
    paradigm: "webhook",
    unit: "mm",
    base: 0.5,
    manufacturer: "structuraltestingtools",
    locations: ["Exterior Wall", "Ceiling", "Pillar"],
    zRange: [0.2, 0.8],
  },
  {
    type: "EpsilonPeak",
    ...
    base: 300,
    manufacturer: "Nerve-Sensors",
    locations: ["Load-bearing Beam", "Pillar"],
    zRange: [0.3, 0.9],
  },
  ...
  // Strain gauges attached to specific materials/systems
  {
    ...
    base: 100,
    manufacturer: "Nerve-Sensors",
    locations: ["Main Water Pipe", "Technical Shaft"],
    zRange: [0.1, 0.9],
  },
  {
    type: "EpsilonFlat",
  ...
  },
  {
    ...
    base: 150,
    manufacturer: "Nerve-Sensors",
    locations: ["Pillar Rebar", "Foundation"],
    zRange: [0.0, 0.5],
  },
  {
    type: "EpsilonGraph",
...
function buildSensors() {
  ...
  ACTIVE_BUILDINGS.forEach((bldg, bIndex) => {
    ...
    const create = (
      ...
      if (!profile) {
        console.warn(
          `⚠️ Skipped: Sensor type '${type}' not found in SENSOR_TYPES.`,
        );
        return;
      }
...
app.get("/api/buildings/active", (req: Request, res: Response) => {
  ...
  if (!active.bounds)
    return res
      .status(503)
      .json({ metadata: { error: true, message: "Initializing" } });
...
app.get("/api/sensors/by-buildings", (req: Request, res: Response) => {
  ...
  const filteredSensors = sensors.filter((s) =>
    selectedBuildingIds.includes(s.buildingId),
  );

  res
    .status(200)
    .json({ sensors: filteredSensors, metadata: { error: false } });
});
  ...
  for (let i = days * 24; i > 0; i--) {
    ...
    fullHistory.push({
      ...
      value: Math.max(
        ...
        sensor.type === "Rain"
          ? Math.random() * 5
          : (SENSOR_TYPES.find((p) => p.type === sensor.type)?.base || 50) +
              noise,
      ),
      unit: sensor.unit,
    });
...
wss.on("connection", (ws: WebSocket) => {
  ...
  const simulateWebhookAlert = () => {
    ...
    if (alertSensors.length > 0) {
      const randomSensor =
        alertSensors[Math.floor(Math.random() * alertSensors.length)];
...
async function startServer() {
  console.log(
    `Fetching structural metadata for ${ACTIVE_BUILDINGS.length} models...`,
  );

  await Promise.all(
    ACTIVE_BUILDINGS.map(async (bldg) => {
      try {
        ...
        const layerData = layerResponse
          ? await layerResponse.json().catch(() => ({}))
          : {};
  ...
  console.log(
    `✅ ${sensors.length} virtual sensors anchored across ${activeCount} active structures.`,
  );
