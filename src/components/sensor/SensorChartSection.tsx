    ...
    <Collapsible open={isOpen} onOpenChange={onToggle} className="flex flex-col gap-3">
      ...
      <div className="flex items-center justify-between">
        <CollapsibleTrigger className="flex items-center gap-3 focus:outline-none">
          ...
          <h3 className="text-sm font-semibold text-foreground">Data Insights</h3>
          {isOpen ? <ChevronUp className="w-4 h-4 text-muted-foreground" /> : <ChevronDown className="w-4 h-4 text-muted-foreground" />}
        </CollapsibleTrigger>
        
        <Select value={chartDays} onValueChange={setChartDays}>
          <SelectTrigger className="w-[80px] h-7 text-xs bg-card border-border text-foreground transition-colors duration-200">
      ...
      <CollapsibleContent className="w-full overflow-hidden space-y-4 pt-1">
        ...
        <div className="bg-card border border-border rounded-xl p-4 shadow-lg w-full overflow-hidden transition-colors duration-200">
          <div className="flex justify-between items-start mb-4">
            <div>
              <div className="text-xs text-muted-foreground mb-1">Avg {sensor.type}</div>
              <div className="text-3xl font-bold text-foreground tracking-tight">
                {avgValue}<span className="text-lg ml-1 font-normal text-muted-foreground">{sensor.unit}</span>
              </div>
            </div>
            
            <div className="flex items-center gap-2">
               <div className="flex bg-muted/50 p-1 rounded-md border border-border">
                  <Button variant="ghost" size="sm" className={`h-6 px-2 py-0 ${chartType === 'line' ? 'bg-background shadow-sm text-foreground' : 'text-muted-foreground hover:text-foreground'}`} onClick={() => setChartType('line')}><LineChartIcon className="w-3 h-3" /></Button>
                  <Button variant="ghost" size="sm" className={`h-6 px-2 py-0 ${chartType === 'bar' ? 'bg-background shadow-sm text-foreground' : 'text-muted-foreground hover:text-foreground'}`} onClick={() => setChartType('bar')}><BarChartIcon className="w-3 h-3" /></Button>
               </div>
               <div className="text-xs font-medium text-destructive bg-destructive/10 px-2 py-1 rounded border border-destructive/20">
                 ↗ +1.2{sensor.unit}
               </div>
            </div>
          </div>
            ...
            <ChartContainer config={{}} className="h-[200px] w-full min-w-0 max-w-full mt-2 overflow-hidden">
                {chartType === 'line' ? (
                  <LineChart data={chartData} margin={{ top: 5, right: 0, bottom: 0, left: 0 }}>
                    ...
                    {/* Note: Kept stroke dynamic based on sensor status */}
                    <Line type="monotone" dataKey="value" stroke={chartColor} strokeWidth={4} dot={false} activeDot={{ r: 6, fill: chartColor, stroke: 'var(--background)', strokeWidth: 2 }} />
                  </LineChart>
                ) : (
